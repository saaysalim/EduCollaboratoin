...
const MyCourses = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render(item) {
        ...
        return courseReg ? courseReg.createdAt.split("T")[0] : "N/A";
      },
    },
    {
      title: "My Status",
  ...
  const myFinalCourses = myCourses.filter((course) => {
    return myCourseRegs.some((cr) => cr.course === course._id);
  });

  const allLessons = useSelector((state) => state.myReducer.lessons);
  const myLessons = allLessons.filter((lesson) => {
    return myCourses.some((course) => course._id === lesson.course);
  });
